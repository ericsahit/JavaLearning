...
public class BlockOutStream extends OutStream {
  ...
  private int mHalfFileBufferBytes = mUserConf.FILE_BUFFER_BYTES / 2;
  ...
  public void write(byte[] b, int off, int len) throws IOException {
    ...
    if (mBuffer.position() + len >= mUserConf.FILE_BUFFER_BYTES) {
      if (mBuffer.position() > 0) {//如果buffer的位置+写入长度>Buffer的最大长度，则将Buffer的内容先写入文件
        appendCurrentBuffer(mBuffer.array(), 0, mBuffer.position());
        mBuffer.clear();
      }
      
      //****此处值得商榷，是否应该先判断len是否大于FILE_BUFFER_BYTES？
      if (len > mHalfFileBufferBytes) {
